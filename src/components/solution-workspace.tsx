...
export function SolutionWorkspace({ scope }: { scope: SolutionScope }) {
  ...
  return (
    <main className="min-h-screen bg-[#070b0f] text-[#f4f1e8]">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_80%_0%,rgba(44,255,173,0.08),transparent_28%),radial-gradient(circle_at_0%_90%,rgba(255,91,52,0.07),transparent_24%)]" />
      <div className="relative mx-auto max-w-5xl px-5 py-8 md:px-8 md:py-12">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-6">
          <div>
            <p className="font-mono text-[10px] tracking-[0.2em] text-[#2cffad]">SOLUTION WALKTHROUGH</p>
            <p className="mt-2 text-xs text-white/40">
              {modeLabels[solution.mode]} · {solution.language === "python" ? "Python" : "C++"}
            </p>
          </div>
          <Link
            className="rounded-lg border border-white/12 bg-white/[0.035] px-4 py-2.5 text-xs font-semibold text-white/65 transition hover:border-white/25 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2cffad]/60"
            href="/"
          >
            ← 문제로 돌아가기
          </Link>
        ...
        <article className="py-9 md:py-12">
            ...
            <ol className="mt-6 grid gap-5">
              {solution.steps.map((step, index) => (
                <li className="grid grid-cols-[2rem_1fr] gap-3" key={`${index}-${step}`}>
                  <span className="grid size-8 place-items-center rounded-full border border-[#2cffad]/25 bg-[#2cffad]/8 font-mono text-xs text-[#7dffcc]">
                    {index + 1}
                  </span>
          ...
          <section className="mt-6 overflow-hidden rounded-2xl border border-white/9 bg-[#090e12]">
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#070b0f] px-5 text-[#f4f1e8]" aria-live="polite">
      <div className="text-center">
        <span className="mx-auto block size-7 animate-spin rounded-full border-2 border-white/15 border-t-[#2cffad]" />
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#070b0f] px-5 text-center text-[#f4f1e8]" aria-live="assertive">
      <div className="max-w-lg">
        ...
        <Link className="mt-7 inline-flex rounded-lg bg-[#2cffad] px-5 py-3 text-sm font-semibold text-[#03140e]" href="/">
          문제로 돌아가기
        </Link>
      </div>
    </main>

...
현재 문제은행에는 이전 기본 문제 6개와 확장 문제 6개를 합친 12개가 있으며, 각 문제는 초보·중수·고수 세 모드와 Python/C++를 모두 지원한다.
...
첫 명령은 12개 문제의 스키마 필수 필드, 난이도 분포, 세 모드 지원, 공개/비공개 경계, 루브릭 가중치와 Python 기준 정답을 확인한다. 두 번째 명령은 로컬 C++17 컴파일러로 C++ 기준 정답까지 검증하고 `artifacts/step2-validation-results.json`에 증빙을 남긴다.

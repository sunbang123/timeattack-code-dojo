...
브라우저 코드에서는 `problem_bank/private`를 import하지 않는다. Step 3의 문제 조회 API는 공개 문서만 직렬화하며, 채점 API만 서버에서 비공개 문서를 읽는다. 저장소가 공개되는 경우 비공개 문서는 운영 전 암호화된 저장소나 접근 제한 저장소로 옮겨야 한다.
...
첫 명령은 스키마 필수 필드, 난이도 분포, 공개/비공개 경계, 루브릭 가중치와 Python 기준 정답을 확인한다. 두 번째 명령은 로컬 C++17 컴파일러로 C++ 기준 정답까지 검증하고 `artifacts/step2-validation-results.json`에 증빙을 남긴다.
